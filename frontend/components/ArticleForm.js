...
export default function ArticleForm(props) {
  ...
  const { postArticle, setCurrentArticleId, currentArticleId, updateArticle, articles, currentArticle, values, setValues} = props
  ...
  const articlePostUpdate = () => {
    // if(currentArticle ){
    //   updateArticle({article_id: currentArticle.article_id, article: values})
    // } else if(values.title && values.text && values.topic){
    //   postArticle(values)
    // }
  ...
  return (
    ...
    <form id="form" onSubmit={onSubmit}>
      ...
      <div className="button-group">
        ...
        <button onClick={() => {setValues(initialFormValues); setCurrentArticleId()}}>Cancel edit</button>
      </div>
    </form>
  )
}
